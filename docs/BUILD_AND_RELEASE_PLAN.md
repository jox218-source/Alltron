...
Evidence date: 2026-09-24. This plan describes the compressed Stage 0–5 sequence in [STAGES.md](STAGES.md). The public preview now includes timers, local commands, shopping state and opt-in speech adapters; Stage 1 remains incomplete and unverified with live audio. A read-only review of the separate private app's 2026-09-23 local speech pilot informs the design below; no Alltron software has been deployed to a Pi or verified as an appliance.
...
| --- | --- | --- |
| Public Alltron timer preview, CI and publication gate | Extend the existing public package and keep its history/audit controls. | Add commands, alarms, lists, speech and owner setup. |
...
| Upstream Whisper/Piper engines and candidate voice profiles | Use pinned upstream dependencies and compare candidates using public, invented test material. | Verify exact licenses, model provenance, ARM64 install and full-load performance before a default is promised. |
...
The shortest dependency path is: settle public contracts and reuse checklist (Stage 0), build local core and speech (Stage 1), integrate HA and Codex as parallel tracks (Stage 2), finish guided installation and simulated acceptance (Stage 3), then run the dedicated Pi gate (Stage 4) and publish beta (Stage 5). UI, documentation, installer scaffolding and fake adapters can start while Stage 1 is in progress. Integration gates cannot close until their actual paths work. The private Pi evidence reduces research effort; it does not waive new-owner installation, privacy or physical acceptance.
...
| 1: local core/speech | UI, typed commands, durable alarms/lists, fake-device harness, invented speech-WAV tests and calibration UI. Narrow speech modules only in a truly isolated Alltron environment. | Command idempotency, one mic owner, cancellation, cue bleed, playback, engine provenance and voice contract. | Local timer/command/voice tests, separate engine health, clean checkout and restart recovery. |
...
| 3: appliance/software acceptance | Wizard, selected audio devices, per-engine checks, installer, diagnostics, backup, rollback and outside-household setup trial. | Helper privileges, idempotency, misleading ready states, recovery and support-bundle privacy. | Clean disposable install, direct-worker/full-path speech smoke, upgrade and failed-update restore. |
| 4–5: physical gate/beta | Fixes and docs from a separate Pi, release prep and independent fresh-owner trial. | Full-load hardware, security, privacy, license, archive and release-candidate review. | Supported profile, measured mic-to-reply path and reviewed beta archive. |
