...
| Private full-release preview | Use only the observed contracts: explicit archive inventory/hashes, staged install/rollback, private HA selection/configuration, and voice asset/health checks. Start equivalent public release modules while Stage 2 integrations finish. | Re-author Alltron code and fictional tests; install HA on the same device, add guided owner setup, prove repeat installs/recovery and review every redistributed asset. The private preview targets a different topology and has no fresh second-Pi acceptance. |
...
The shortest dependency path now is: Stages 0–1 and the Stage 2 prototype (merged); finish disposable HA authorization and isolated Codex trial (Stage 2) **while** building Alltron's archive, preflight and rollback modules (Stage 3); join them for a clean disposable Linux install and independent software setup trial; then run dedicated-Pi live speech/full-stack acceptance (Stage 4) before beta (Stage 5). Wake integration, resident Piper, real microphone/speaker selection, spoken end-to-end checks and ARM64 performance remain Stage 4 gates. The private preview shortens design discovery, not public implementation, licensing review or acceptance.
