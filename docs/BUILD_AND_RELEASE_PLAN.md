...
Evidence date: 2026-09-26. This plan describes the revised Stage 0–5 sequence in [STAGES.md](STAGES.md). The public preview includes timers, local commands, shopping state, opt-in speech/HA adapters and a fake-tested Codex transport. Stages 0–1 and the Stage 2 prototype are merged; source packaging and local preview recovery are in the open PR #9 integration stack. Live audio work remains in Stage 4 because no dedicated test Pi is available. Read-only reviews of the separate private speech pilot and full-release preview inform the design below; no Alltron software has been deployed to a Pi or verified as an appliance.
...
| Private full-release preview | Use only the observed contracts: explicit archive inventory/hashes, staged install/rollback, private HA selection/configuration, and voice asset/health checks. Alltron's open PR #9 now includes a public source archive and local preview install/recovery tooling. | Complete disposable Linux service and HA installation, guided owner setup, repeat installs/recovery and review every redistributed asset. The private preview targets a different topology and has no fresh second-Pi acceptance. |
...
The shortest dependency path now is: Stages 0–1 and the Stage 2 prototype (merged); finish disposable HA authorization and isolated Codex trial (Stage 2) **while** extending Stage 3's source archive and local preview recovery into a disposable Linux service/HA installer; join them for a clean Linux install and independent software setup trial; then run dedicated-Pi live speech/full-stack acceptance (Stage 4) before beta (Stage 5). Wake integration, resident Piper, real microphone/speaker selection, spoken end-to-end checks and ARM64 performance remain Stage 4 gates. The private preview shortens design discovery, not public implementation, licensing review or acceptance.
