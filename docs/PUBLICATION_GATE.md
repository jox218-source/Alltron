...
The repository's original commit `b400f03` was public before this gate and has the owner's personal email in Git author metadata. On 2026-09-22 the owner chose to retain that history and document the exposure. The script treats that one commit as the fixed public base and requires no-reply identities for later commits. This does **not** erase or hide the existing exposure; copies already fetched or cached elsewhere remain possible.

## Release gate

Before any release, repeat the branch gate on the exact tag/commit and inspect the built archive, installer, package metadata and licenses. Verify GitHub secret scanning, push protection, Dependabot security updates and private vulnerability reporting remain enabled. Do not publish a beta until the dedicated Pi and independent owner setup gates in [STAGES.md](STAGES.md) pass.
